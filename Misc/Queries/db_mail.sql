...
exec  msdb.dbo.sp_update_operator  @name = @operator, @email_address = 'user2@example.com;user3@example.com'
...
@action = N'REASSIGN',
@current_owner_login_name = N'DOMAIN\job.owner',
...
WHERE l.name like 'DOMAIN\job.owner'
ORDER by l.name
--===================
...
exec msdb.dbo.sp_send_dbmail @recipients='user2@example.com;user3@example.com',@subject='testmail 321'

EXECUTE msdb.dbo.sysmail_update_account_sp    @account_name = 'noreply@example.com' ,    @display_name = '' ,       @replyto_address =  'user2@example.com' ,      @description = ''
